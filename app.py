...
# Não será usada no Front end, por isso será desabilitada.
...
def get_canoas_por_municipio(query: SchemaBuscaCanoaPorMunicipio):
    ...
    query = session.query(Canoa.id, Canoa.nome, Canoa.tipo, Canoa.dono, Canoa.telefone,
                      Canoa.local, Localidade.estado, Localidade.municipio,
                      Localidade.bairro, Localidade.referencia)
    query = query.join(Localidade, Canoa.local == Localidade.id)
    query = query.filter(Localidade.municipio == local_buscado)
    canoas = query.all()
    ...
    if not canoas:
        ...
        # retorna a representação de produto
        return apresenta_canoas_e_localidades(canoas), 200
        #return apresenta_canoas(canoas), 200
